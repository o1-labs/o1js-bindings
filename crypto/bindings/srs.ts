...
function srsPerField(f: 'fp' | 'fq', wasm: Wasm, conversion: RustConversion) {
  ...
  let lagrangeCommitmentsWholeDomain = (srs: WasmSrs, domain_size: number) =>
    wasm[`caml_${f}_srs_lagrange_commitments_whole_domain`](
      srs,
      domain_size
    );
  ...
  return {
    ...
    lagrangeCommitmentsWholeDomain(srs: WasmSrs, domainSize: number) {
      let wasmCommsOpt = lagrangeCommitmentsWholeDomain(srs, domainSize);
      // Always supposing it is not undefined, i.e. no error raised.
      let mlComms = conversion[f].polyCommsFromRust(wasmCommsOpt);
      return mlComms;
    },
